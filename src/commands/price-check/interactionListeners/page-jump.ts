...
client.on("interactionCreate", async (interaction) => {
    ...
    try {
        ...
        const pageInput = interaction.fields.getTextInputValue("price-check:page:jump-input");
        const pageNumber = parseInt(pageInput);
        const priceCheckSequence = PriceCheckSequence.getSequence({ username: interaction.user.username });
        const { visualContainer } = await priceCheckSequence.navigateToPage(pageNumber);
