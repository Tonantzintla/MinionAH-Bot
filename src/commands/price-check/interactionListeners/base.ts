...
client.on("interactionCreate", async (interaction) => {
  ...
  try {
    ...
    const typeFilter = interaction.options.get("type")?.value as
      | string
      | undefined;
    const tierFilter = interaction.options.get("tier")?.value as
      | number
      | undefined;
    ...
    await interaction.reply({
      components: [(await priceCheckSequence.getCurrentPage()).visualContainer],
