...
client.on("interactionCreate", async (interaction) => {
    ...
    try {
        ...
        let currentPageOffset = 0, targetPage;
        switch (action) {
            case "previous":
                currentPageOffset = -1;
                break;
        ...
        const { visualContainer } = await priceCheckSequence.navigateToPage(priceCheckSequence.currentPageNumber + currentPageOffset);
