 ...
 */
async function getMinionEmbed(
  minionPrices: Awaited<ReturnType<typeof getMinionPrices>>,
  offset: number = 0
) {
  try {
    ...
    const displayedEntries = Object.entries(minionPrices).slice(
 ...
 */
function constructLocalPagination(
  pageNumber: number,
  maxPages: number,
  filter: string = "_none",
  tier: number = -1
  ...
  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
    ...
    new ButtonBuilder()
      .setCustomId(
        `price-check:prices:move-to:${pageNumber - 1}:${filter}:${tier}:local:${crypto.randomBytes(6).toString("hex")}`
      )
      .setEmoji("⬅️")
      .setStyle(ButtonStyle.Primary)
...
function constructSecondaryPagination(
  pageNumber: number,
  maxPages: number,
  filter: string = "_none",
  tier: number = -1
) {
  function constructSecondaryPaginationButton(
    lowLimit: number,
    displacement: number
  ) {
    ...
    return new ButtonBuilder()
      .setCustomId(
        pageNumber === lowLimit
          ? `price-check:prices:paginationModal:${filter}:${tier}:secondary:${crypto.randomBytes(6).toString("hex")}`
          : directionID
      )
  ...
  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
    ...
    new ButtonBuilder()
      .setCustomId(
        pageNumber < 2
          ? `price-check:prices:move-to:2:${filter}:${tier}:secondary:${crypto.randomBytes(6).toString("hex")}`
          : `price-check:prices:paginationModal:${filter}:secondary:${crypto.randomBytes(6).toString("hex")}`
      )
...
client.on("interactionCreate", async (interaction) => {
  ...
  try {
    ...
    // get prices
    const filter =
      (interaction.options.get("type")?.value as string | undefined) ?? "_none";
    const tier =
      (interaction.options.get("tier")?.value as number | undefined) ?? -1;
...
client.on("interactionCreate", async (interaction) => {
  if (!interaction.isButton()) return;
  if (
    !interaction.customId.startsWith("price-check:prices:paginationModal") ||
    !interaction.customId.includes(":secondary")
  )
    return;
  try {
    // get the filter
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (
    !interaction.customId.startsWith(
      "price-check:prices:paginationModal_instance"
    )
  )
    return;
  try {
    ...
    const pageNumber = parseInt(
      interaction.fields.getTextInputValue(
        "price-check:prices:paginationModal_instance:pageInput"
      )
    );
    // get the filter
    const filter = interaction.customId.split(":")[3];
    ...
    if (
      pageNumber < 1 ||
      pageNumber > Math.ceil(Object.keys(minionPrices).length / minionsPerPage)
    ) {
      await interaction.reply({
