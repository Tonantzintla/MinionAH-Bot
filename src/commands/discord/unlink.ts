...
export default new SlashCommandSubcommandBuilder().setName("unlink").setDescription("Unlink your Discord account from your MinionAH account");
...
client.on("interactionCreate", async (interaction) => {
  ...
  try {
    ...
    const warningEmbed = new EmbedBuilder().setTitle("⚠️ Unlink your Discord account from MinionAH").setColor("#262626").setDescription("Are you sure you want to unlink your Discord account from MinionAH? You can link it back at any time by using the `/discord link` command");
...
client.on("interactionCreate", async (interaction) => {
  ...
  try {
    ...
    if (interaction.customId.startsWith("discord-integration:unlink-cancel")) {
      const responseEmbed = new EmbedBuilder().setTitle("🚫 Unlinking Process Cancelled").setColor("#262626").setDescription("The unlinking process has been cancelled");
    ...
    const responseEmbed = new EmbedBuilder().setTitle("✅ Unlinking Process Completed ").setColor("#262626").setDescription("Your Discord account has been unlinked from MinionAH. You can link it back at any time by using the `/discord link` command");
    await interaction.reply({ embeds: [responseEmbed], components: [], ephemeral: true });
  } catch (error) {
    console.error(error);
    await interaction.reply({ content: "There was an error while executing this command!", ephemeral: true });
