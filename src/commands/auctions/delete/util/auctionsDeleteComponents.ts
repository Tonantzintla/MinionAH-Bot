...
const auctionNotFoundDisplayContainer = new ContainerBuilder()
  .addTextDisplayComponents((t) => t.setContent("### Auction Not Found"))
...
async function constructAuctionDeleteConfirmationDisplayContainer(
  auction: Auction
) {
  ...
  return new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      ...
      (t) =>
        t.setContent(`**Free Will:** ${auction.hasFreeWill ? "Yes" : "No"}`),
      (t) =>
        t.setContent(`**Negotiable:** ${auction.isNegotiable ? "Yes" : "No"}`)
    )
    .addSeparatorComponents((s) => s.setSpacing(SeparatorSpacingSize.Large))
    .addTextDisplayComponents((t) =>
      t.setContent(`*By [minionah.com](https://minionah.com).*`)
