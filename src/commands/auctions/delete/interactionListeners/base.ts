...
client.on("interactionCreate", async (interaction) => {
    ...
    try {
        ...
        if (!auction) {
            await interaction.reply({
                components: [auctionsDeleteComponents.static.auctionNotFoundDisplayContainer],
                flags: [MessageFlags.Ephemeral, MessageFlags.IsComponentsV2]
            });
            return;
        }

        const confirmationComponent = await auctionsDeleteComponents.constructors.constructAuctionDeleteConfirmationDisplayContainer(auction)
        if (!confirmationComponent) throw new Error("Failed to construct auction delete confirmation component");

        kv.set(`auction:delete:pending:${interaction.user.username}`, auction.id, 15 * 60);
