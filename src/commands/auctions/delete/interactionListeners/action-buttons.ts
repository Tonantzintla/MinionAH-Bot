...
client.on("interactionCreate", async (interaction) => {
    ...
    try {
        ...
        const targetAuctionID = kv.get<string>(`auction:delete:pending:${interaction.user.username}`);
        if (!targetAuctionID) {
            await interaction.reply({
                components: [auctionsDeleteComponents.static.pendingDeletionExpiredDisplayContainer],
        ...
        switch (action) {
            case "confirm": {
                ...
                await interaction.reply({
                    components: [auctionsDeleteComponents.static.auctionDeletionSuccessDisplayContainer],
                    flags: [MessageFlags.Ephemeral, MessageFlags.IsComponentsV2]
                });
                break;
            }
            case "cancel": {
                kv.del(`auction:delete:pending:${interaction.user.username}`);
                await interaction.reply({
                    components: [auctionsDeleteComponents.static.auctionDeletionCancelledDisplayContainer],
                    flags: [MessageFlags.Ephemeral, MessageFlags.IsComponentsV2]
                });
                break;
            }
