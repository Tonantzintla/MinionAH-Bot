...
import { invalidAuctionCreateDisplayContainer, noLinkedMinionAHAccountFoundDisplayContainer } from "../util/staticComponents";
...
client.on("interactionCreate", async (interaction) => {
    ...
    const linkedMinionAHUser = await getLinkedMinionAHUser(interaction.user.username);
    ...
    try {
        ...
        await interaction.reply({ components: [invalidAuctionCreateDisplayContainer], flags: [MessageFlags.Ephemeral, MessageFlags.IsComponentsV2] });
    }
