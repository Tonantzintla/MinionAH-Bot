...
export default class AuctionCreator {
  ...
  public constructor(private interaction: AuctionCreateInteraction) {
    const optionData = auctionCreatorSettings.options
      .map((optionName) => this.interaction.options.get(optionName))
      .filter((opt) => opt !== null);
    const auctionOptions = Object.fromEntries(
      optionData.map((opt) => [opt!.name, opt!.value])
    );
  ...
  public static async confirmCreation(username: string) {
    ...
    const minionAHUser = await getLinkedMinionAHUser(
    ...
    if (!minionData) {
      throw new Error(
        "Invalid minion type or tier provided for auction creation."
      );
    }
  ...
  public async getConfirmationDisplayContainer() {
    ...
    return new ContainerBuilder()
      ...
      .addTextDisplayComponents(
        ...
        (t) =>
          t.setContent(
            `**Price per Minion:** ${formatMinionPrice(this.auctionOptions.price)} coins`
          ),
        (t) =>
          t.setContent(
            `**Mithril Infusion:** ${this.auctionOptions.mithril_infusion ? "Yes" : "No"}`
          ),
        (t) =>
          t.setContent(
            `**Free Will:** ${this.auctionOptions.free_will ? "Yes" : "No"}`
          ),
        (t) =>
          t.setContent(
            `**Negotiable:** ${this.auctionOptions.negotiable ? "Yes" : "No"}`
          )
      )
      .addSeparatorComponents((s) => s.setSpacing(SeparatorSpacingSize.Large))
