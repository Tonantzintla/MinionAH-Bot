...
async function getAuctions(
  ...
  return repeatArray(
    ...
    .map((auction, index) => ({ ...auction, auctionID: `${index + 1}` }))
    .slice(0, 25);
}
  ...
  try {
    ...
    if (auctions.length === 0) {
      await interaction.reply({
        content: "You don't have any auctions to delete!",
        ephemeral: true
      });
      return;
    }
    ...
    if (!systemEmojis) {
      await interaction.reply({
        content:
          "Could not fetch emojis...? Hey, please contact @andriotis. He'll get to it immediately",
        ephemeral: true
      });
      return;
    }
      ...
      .addOptions(
        auctions.map((auction) =>
          new StringSelectMenuOptionBuilder()
            ...
            .setLabel(
              `${auction.amount}x ${parseMinionType(auction.type)} for ${auction.price.toLocaleString()} coins`
            )
            .setDescription(
              `Tier ${auction.tier} | ${auction.freeWill ? "Free Will" : ""} | ${auction.mithrilInfused ? "Mithril Infused" : ""}`
            )
        )
      );
    ...
    await interaction.reply({
      content: "Select an auction to delete",
