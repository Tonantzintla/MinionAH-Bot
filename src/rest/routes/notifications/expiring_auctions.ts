...
export default async function expiring_auctions(req: e.Request, res: e.Response) {
    try {
        const { auctionIDs, receiverDiscordID } = req.body as z.infer<typeof notif_expiring_auctions_schemaZod>;
...
async function getNotificationEmbed(auctionIDs: string[], receiverDiscordID: string) {
