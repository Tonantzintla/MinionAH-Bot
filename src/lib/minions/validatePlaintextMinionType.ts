 ...
 */
export default async function validatePlaintextMinionType(
  type: string,
  tier: number
) {
  try {
    ...
    const validMinionIDs = minionIDs.filter(
      (ID) =>
        ID.includes(type.toUpperCase()) &&
        ID.split("_").slice(-1)[0] === tier.toString()
    );
    return {
      systemError: false,
